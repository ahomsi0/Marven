    ...
    <div className="flex h-full min-w-0 flex-col bg-[#1e1e1e]">
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          {/* Code area */}
          <div className="flex min-h-0 flex-1 overflow-hidden">
            <div className="w-10 shrink-0 select-none border-r border-[#2a2a2a] bg-[#1a1a1a] py-3 text-right font-mono text-[11px] leading-7 text-[#555] pr-2">
              {fileContent.split("\n").slice(0, 50).map((_, i) => (
                <div key={i}>{i + 1}</div>
              ))}
            </div>
            <textarea
              ...
              spellCheck={false}
              className="agent-editor h-full min-h-full w-full resize-none border-0 bg-[#1e1e1e] px-4 py-3 font-mono text-[12px] leading-7 text-[#d4d4d4] outline-none disabled:opacity-40"
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between border-t border-[#333] bg-[#1a1a1a] px-3 py-1 font-mono text-[9px] text-[#666]">
        ...
        <div className="flex gap-4">
          ...
          <span className="text-[#d19a66]/50">TypeScript</span>
        </div>
      </div>
    </div>
  );
